...
contains
  ...
  subroutine evolucion(phi, gama, dx, dt, npts)
    integer, intent(in) :: npts
    real(dp), intent(in) :: dx, dt
    complex(dp), intent(inout) :: phi(0:npts)
    ...
    ! Asignación dinámica dentro del bucle temporal (Cuello de botella)
    allocate(beta(0:npts))
...
program cayley
  ...
  complex(dp), allocatable :: gama(:), phi(:)

  ! Lectura automatizada para scripts
  read(5,*) archivo
  read(5,*) ancho
  read(5,*) p
  read(5,*) v0
  ...
  npts = 2**log2n
  allocate(x(0:npts), v(0:npts), gama(0:npts), phi(0:npts))
