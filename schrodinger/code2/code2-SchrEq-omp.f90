...
contains
  ...
  subroutine tridiag(npts, dx, dt, gama, v)
    ...
    ! Bucle secuencial estricto por recurrencia
    do ix = npts - 1, 0, -1
  ...
  subroutine evolucion(phi, gama, beta, dx, dt, npts)
    ...
    ! Bucle secuencial estricto (backward substitution)
    do ix = npts - 2, 0, -1
      beta(ix) = gama(ix + 1) * (beta(ix + 1) - (4.0_dp * ci * dx**2 / dt) * phi(ix + 1))
    end do
    
    chi = (0.0_dp, 0.0_dp)
    ! Bucle secuencial estricto (forward substitution)
    do ix = 1, npts - 1
...
program cayley
  ...
  ! Paralelización de la fase de setup (dominio, potencial y función de onda)
  !$omp parallel do private(i) shared(x, v, phi, L, dx, npts, v0, x0, p, ancho) default(none)
  do i = 1, npts
  ...
  do it = 1, nt
    tiempo = tiempo + dt
    ! La evolución no se puede paralelizar con !$omp do debido a dependencias tridiagonales.
