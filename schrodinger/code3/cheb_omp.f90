...
contains
  ...
  subroutine apply_h_omp(phi, h_phi, v, dx, npts)
    ...
    !$omp parallel do private(i) shared(h_phi, phi, v, dx2_inv, npts) default(none)
    do i = 1, npts - 1
...
program chebyshev_fd_omp
  use math_utils_cheb
  implicit none
  integer :: npts, nt, i, j, it, k, m_cheb, log2n, B_dummy
  ...
  !$omp parallel do private(i) shared(x, v, phi, L, dx, v0, x0, p, ancho, npts) default(none)
  do i = 0, npts
    x(i) = -L/2.0_dp + dx * i
    v(i) = v_pot(x(i), v0)
  ...
  do it = 1, nt
    ...
    !$omp parallel do private(i) shared(phi_0, phi, psi_new, c_k, npts) default(none)
    do i = 0, npts
      phi_0(i) = phi(i)
      psi_new(i) = c_k * phi_0(i)
    end do
    
    call apply_h_omp(phi_0, h_phi, v, dx, npts)
    
    c_k = 2.0_dp * (-ci) * bessel_jn(1, alpha)
    !$omp parallel do private(i) shared(phi_1, h_phi, e_avg, phi_0, de_inv, psi_new, c_k, npts) default(none)
    do i = 0, npts
    ...
    do k = 2, m_cheb
      call apply_h_omp(phi_1, h_phi, v, dx, npts)
      c_k = 2.0_dp * (-ci)**k * bessel_jn(k, alpha)
      
      !$omp parallel do private(i) shared(phi_2, h_phi, e_avg, phi_1, de_inv, phi_0, psi_new, c_k, npts) default(none)
      do i = 0, npts
    ...
    !$omp parallel do private(i) shared(phi, psi_new, phase_shift, npts) default(none)
    do i = 0, npts
