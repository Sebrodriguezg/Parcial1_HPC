...
contains
  ...
  subroutine apply_h_omp(phi, h_phi, v, dx, npts)
    ...
    !$omp parallel do private(i) shared(h_phi, dx2_inv, phi, v, npts) default(none)
    do i = 1, npts - 1
      h_phi(i) = -0.5_dp * dx2_inv * (phi(i+1) - 2.0_dp * phi(i) + phi(i-1)) + v(i) * phi(i)
    end do
  end subroutine apply_h_omp
...
program lanczos_fd
  ...
  implicit none
  integer :: log2n, npts, nt, i, j, it, k, m_lanczos
  real(dp) :: dt, dx, L, tiempo, x0, ancho, p, v0, norm_psi
  ...
  complex(dp), parameter :: ci = (0.0_dp, 1.0_dp)
  complex(dp) :: dot_prod_local
  real(dp) :: dot_prod_real, dot_prod_imag
  ...
  !$omp parallel do private(i) shared(x, v, psi, L, dx, v0, x0, p, ancho, npts) default(none)
  do i = 0, npts
    x(i) = -L / 2.0_dp + i * dx
    v(i) = v_pot(x(i), v0)
  ...
  do it = 1, nt
    ...
    !$omp parallel do reduction(+:dot_prod_real, dot_prod_imag) private(i, dot_prod_local) shared(psi, npts) default(none)
    do i = 1, npts-1
    ...
    !$omp parallel do private(i) shared(v_mat, psi, norm_psi, npts) default(none)
    do i = 0, npts
      v_mat(i, 1) = psi(i) / norm_psi
    end do
    
    t_mat = 0.0_dp
    
    do j = 1, m_lanczos
      call apply_h_omp(v_mat(:,j), h_v, v, dx, npts)
      
      if (j == 1) then
        !$omp parallel do private(i) shared(w, h_v, npts) default(none)
        do i = 0, npts
          w(i) = h_v(i)
        end do
      else
        !$omp parallel do private(i) shared(w, h_v, beta, v_mat, j, npts) default(none)
        do i = 0, npts
          w(i) = h_v(i) - beta(j-1) * v_mat(i, j-1)
        end do
      end if
      
      dot_prod_real = 0.0_dp
      dot_prod_imag = 0.0_dp
      !$omp parallel do reduction(+:dot_prod_real, dot_prod_imag) private(i, dot_prod_local) shared(v_mat, w, j, npts) default(none)
      do i = 1, npts-1
        dot_prod_local = conjg(v_mat(i,j)) * w(i)
        dot_prod_real = dot_prod_real + real(dot_prod_local, dp)
        dot_prod_imag = dot_prod_imag + aimag(dot_prod_local)
      end do
      alpha(j) = dot_prod_real * dx
      
      !$omp parallel do private(i) shared(w, alpha, v_mat, j, npts) default(none)
      do i = 0, npts
        w(i) = w(i) - alpha(j) * v_mat(i,j)
      end do
      
      dot_prod_real = 0.0_dp
      dot_prod_imag = 0.0_dp
      !$omp parallel do reduction(+:dot_prod_real, dot_prod_imag) private(i, dot_prod_local) shared(w, npts) default(none)
      do i = 1, npts-1
        dot_prod_local = conjg(w(i)) * w(i)
        dot_prod_real = dot_prod_real + real(dot_prod_local, dp)
        dot_prod_imag = dot_prod_imag + aimag(dot_prod_local)
      end do
      beta(j) = sqrt(dot_prod_real * dx)
      
      t_mat(j, j) = alpha(j)
      if (j < m_lanczos) then
        t_mat(j, j+1) = beta(j)
        t_mat(j+1, j) = beta(j)
        !$omp parallel do private(i) shared(v_mat, w, beta, j, npts) default(none)
        do i = 0, npts
          v_mat(i, j+1) = w(i) / beta(j)
        end do
      end if
    end do
    ...
    !$omp parallel do private(i) shared(psi, npts) default(none)
    do i = 0, npts
      psi(i) = (0.0_dp, 0.0_dp)
    end do
    
    do j = 1, m_lanczos
      !$omp parallel do private(i) shared(psi, v_mat, y_vec, j, npts) default(none)
      do i = 0, npts
        psi(i) = psi(i) + v_mat(i,j) * y_vec(j)
      end do
    end do
    
    !$omp parallel do private(i) shared(psi, norm_psi, npts) default(none)
    do i = 0, npts
    ...
    if (mod(it, 100) == 0) then
      do j = 0, npts, 5
        write(10, '(5F15.7)') tiempo, x(j), abs(psi(j)), v_pot(x(j), v0) / (2.0_dp * v0), norm_psi
      end do
      write(10, *)
      write(10, *)
    end if
