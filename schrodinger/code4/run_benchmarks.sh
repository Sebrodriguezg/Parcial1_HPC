...
SRC_ORIGINAL="code4-base.f90"
SRC_CACHE="code4-cache.f90"

DATOS_SALIDA="datos_tiempos.csv"

# ====================================================================
# Creación de carpetas para las salidas .dat
# ====================================================================
...
echo "=== Iniciando Benchmarking (Lanczos): Original vs Optimizado ==="
...
for config in "${CODIGOS[@]}"; do
    ...
    # Asignar carpeta de salida según versión
    if [ "$VERSION" == "Original" ]; then
        DIR_OUT=$DIR_ORIGINAL
    else
        DIR_OUT=$DIR_CACHE
    fi
        ...
        for log2n in "${TALLAS_LOG2N[@]}"; do
            ...
            for dt in "${VALORES_DT[@]}"; do
                ...
                for B in "${ARREGLO_B[@]}"; do
                    ...
                    # Se incluye la carpeta DIR_OUT en el nombre del archivo
                    ARCHIVO_ONDAS="${DIR_OUT}/ondas_${VERSION}_${bandera}_N${N}_dt${dt}_B${B}.dat"
...
echo "=== Los archivos .dat están en: $DIR_ORIGINAL/ y $DIR_CACHE/ ==="
