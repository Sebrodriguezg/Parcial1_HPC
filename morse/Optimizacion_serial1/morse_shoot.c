...
double fun_wave(double en, double x0, double xf, double h, int n, double D, double alpha) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        /* Normalización preventiva para evitar desbordamiento */
        if (fabs(psi) > 1e15) {
            phi /= 1e15;
            psi /= 1e15;
        }
...
int main(int argc, char *argv[]) {
    ...
    /* Búsqueda de niveles (Disparo + Bisección) */
    ...
    printf("%8d %20.12f %20.12f %20.12f %20.12f %20.12f\n", 
            nsteps, en_levels[0], en_levels[1], en_levels[2], en_levels[3], time_used);
